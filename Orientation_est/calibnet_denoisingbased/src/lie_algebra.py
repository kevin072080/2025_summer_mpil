...
class SO3:
    ...
    def exp(cls, phi):
        ...
        elif phi.ndim == 3: # B, T, 3
            eps = 1e-8
            I = torch.eye(3, device=phi.device, dtype=phi.dtype).view(1, 1, 3, 3)  # [1,1,3,3]
            angle = phi.norm(dim=-1, keepdim=True)  # [B,T,1]
            mask_small = (angle < cls.TOL)  # [B,T,1]

            a = angle.clamp_min(eps)
            axis = phi / a  # [B,T,3]
            K = cls.wedge(axis)  # [B,T,3,3]
            K2 = K @ K  # [B,T,3,3]
            outer = cls.bouter(axis, axis)  # [B,T,3,3]

            c = torch.cos(angle)  # [B,T,1]
            s = torch.sin(angle)  # [B,T,1]
            c = c.unsqueeze(-1)  # [B,T,1,1]
            s = s.unsqueeze(-1)  # [B,T,1,1]

            # Rodrigues
            R_rodrigues = c * I + (1.0 - c) * outer + s * K  # [B,T,3,3]
            # 소각 근사 (I + K + 0.5 K^2)
            R_small = I + K + 0.5 * K2

            # 브랜치 없이 합치기
            mask4 = mask_small.unsqueeze(-1)  # [B,T,1,1]
            Rot = torch.where(mask4, R_small, R_rodrigues)
            '''angle = phi.norm(dim=2, keepdim=True) # B, T, 1
            mask = angle[:, :, 0] < cls.TOL
            dim_batch = phi.shape[0]
            dim_ts = phi.shape[1]
            Id = cls.Id.expand(dim_batch, dim_ts, 3, 3)

            axis = phi[~mask] / angle[~mask] # normalize (angle < tolerance)
            c = angle[~mask].cos().unsqueeze(-1).unsqueeze(-1)
            s = angle[~mask].sin().unsqueeze(-1).unsqueeze(-1)

            Rot = phi.new_empty(dim_batch, dim_ts, 3, 3)
            Rot[mask] = Id[mask] + SO3.wedge(phi[mask])
            Rot[~mask] = c * Id[~mask] + \
                         (1 - c) * cls.bouter(axis, axis) + s * cls.wedge(axis)'''
    ...
    def log(cls, Rot):
        ...
        cos_angle = (0.5 * cls.btrace(Rot) - 0.5).clamp(-1., 1.)  # (tr(R)-1)/2
        angle = cos_angle.acos()
        mask = angle < cls.TOL

        # 기본값: 작은 각도 근사 (1st-order)
        # log(R) ~ 0.5 * (R - R^T)^\vee
        phi = cls.vee(0.5 * (Rot - Rot.transpose(1, 2)))

        # 작은 각도가 아닌 곳만 정식식으로 덮어쓰기
        if (~mask).any():
            a = angle[~mask].unsqueeze(1).unsqueeze(2)
            factor = 0.5 * a / a.sin()  # θ/(2 sinθ)
            phi[~mask] = cls.vee(factor * (Rot[~mask] - Rot[~mask].transpose(1, 2)))
    ...
    def wedge(phi):
        if phi.ndim == 2:
            ...
            dim_batch = phi.shape[0]
            dim_ts = phi.shape[1]
            zero = phi.new_zeros(dim_batch, dim_ts)

            return torch.stack((zero, -phi[:, :, 2], phi[:, :, 1],
                                phi[:, :, 2], zero, -phi[:, :, 0],
                                -phi[:, :, 1], phi[:, :, 0], zero), 1).view(dim_batch, dim_ts,
                                                                            3, 3)
    ...
    def bouter(vec1, vec2):
        ...
        if vec1.ndim == 2:
            ...
            return torch.einsum('bti,btj -> btij', vec1, vec2)
