...
from src.Ori_visualize import ViewerSimple as VS
...
class LearningBasedProcessing:
    ...
    def loop_train(self, dataloader, optimizer, criterion):
        ...
        loss_epoch = 0
        optimizer.zero_grad()
        for us, xs, t in dataloader:
            # print(us.shape, xs.shape)
            us = dataloader.dataset.add_noise(us.cuda())
            hat_xs = self.net(us) # hat_xs : completely compensated gyro
            #print(us.shape, xs.shape, hat_xs.shape)
            loss = criterion(xs.cuda(), hat_xs)/len(dataloader) # xs 는 16 차이나는 시퀀스끼리의 각도 차이
            loss.backward()
    ...
    def loop_val(self, dataset, criterion):
        ...
        with torch.no_grad():
            for i in range(len(dataset)):
                us, xs, t = dataset[i]
                #print(us.shape, xs.shape)
                #print(us.cuda().unsqueeze(0).shape)
    ...
    def loop_test(self, dataset, criterion):
        ...
        for i in range(len(dataset)):
            ...
            with torch.no_grad():
                hat_xs = self.net(us.cuda().unsqueeze(0))
            # print(xs.cuda().unsqueeze(0).shape, hat_xs.shape)
            loss = criterion(xs.cuda().unsqueeze(0), hat_xs)
...
class GyroLearningBasedProcessing(LearningBasedProcessing):
    ...
    def display_test(self, dataset, mode):
        ...
        for i, seq in enumerate(dataset.sequences):
            print('\n', 'Results for sequence ' + seq ) # V1_01_easy ...
            self.seq = seq
            # get ground truth
            ...
            #plt.show()
        file_path_ori = os.path.join(self.address, "orientation_Denoising.json")
        file_path_metric = os.path.join(self.address, "metric_Denoising.json")
    ...
    def plot_gyro(self, seq):
        ...
        resultdict_ori = {
            ...
            'qx': SO3.to_quaternion(net_Rots)[:, 1].tolist(),
            'qy': SO3.to_quaternion(net_Rots)[:, 2].tolist(),
            'qz': SO3.to_quaternion(net_Rots)[:, 3].tolist(),
            'qw': SO3.to_quaternion(net_Rots)[:, 0].tolist(),
        }
        #self.to_save_gt_q(seq, N)

        path = os.path.join(self.address, seq, seq + "_ori_q.txt")  # 확장자 앞에 +가 빠져서 수정
        ori = np.concatenate([np.array(self.t).reshape(-1, 1), SO3.to_quaternion(net_Rots)[:, [1, 2, 3, 0]].cpu()], axis=1)
    ...
    def to_save_gt_q(self, seq, N):
        path_gt_q = '/home/mpil/astrobee_dataset_revised/'
        gt = self.gt['Rots'][:N].cuda()
        ori_gt =  np.concatenate([np.array(self.t).reshape(-1, 1), SO3.to_quaternion(gt)[:, [1, 2, 3, 0]].cpu()], axis=1)
        np.savetxt(os.path.join(path_gt_q, seq, seq + '_gt_q.txt'), ori_gt, delimiter=" ", fmt='%1.9f')
    ...
    def plot_orientation_error(self, seq, imu_Rots, net_Rots, N):
        gt = self.gt['Rots'][:N].cuda()
        # gt = SO3.dnormalize(gt)
        # net_Rots = SO3.dnormalize(net_Rots)
        '''viewer = VS(gt_data=gt, est_data=net_Rots)
        viewer.connect()
        viewer.set_mesh()
        viewer.update(0)
        viewer.pause()
        viewer.disconnect()'''
        raw_err = 180/np.pi*SO3.log(bmtm(gt, imu_Rots)).cpu() # bmtm : bji, bjk -> bik => rot_est.inv() * rot_gt on degree
        net_err = 180/np.pi*SO3.log(bmtm(gt, net_Rots)).cpu() #torch.Size([28704, 3])
