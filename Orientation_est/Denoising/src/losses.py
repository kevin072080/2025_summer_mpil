...
class GyroLoss(BaseLoss):
    ...
    def forward_with_rotation_matrices(self, xs, hat_xs):
        ...
        #######################################################################################################
        #w_yaw_loss = torch.tensor([1., 1., 0.], device=rs.device, dtype=rs.dtype).reshape(1, 1, 3)
        #rs_yaw_loss = rs * w_yaw_loss
        ...
        for k in range(self.min_N, self.max_N):
            ...
            Xs = Xs[::2].bmm(Xs[1::2])
            rs = SO3.log(bmtm(Omegas, Xs)).reshape(N, -1, 3)[:, self.N0:]
            #w2_yaw_loss = torch.tensor([1., 1., 0.], device=rs2.device, dtype=rs2.dtype).reshape(1, 1, 3)
            #rs2_yaw_loss = rs2 * w2_yaw_loss
            loss2 = loss + self.f_huber(rs) / (2 ** (k - self.min_N + 1))
        return loss2
        '''N = xs.shape[0]
        Xs = SO3.exp(xs[:, ::self.min_train_freq].reshape(-1, 3).double())
        hat_xs = self.dt*hat_xs.reshape(-1, 3).double()
        ...
        w_yaw_loss = torch.tensor([1.,1.,0.], device=rs.device,dtype=rs.dtype).reshape(1,1,3)
        rs_yaw_loss = rs * w_yaw_loss
        loss = self.f_huber(rs_yaw_loss)
        # compute increment from min_train_freq to max_train_freq
        for k in range(self.min_N, self.max_N):
            Omegas = Omegas[::2].bmm(Omegas[1::2])
            Xs = Xs[::2].bmm(Xs[1::2])
            rs2 = SO3.log(bmtm(Omegas, Xs)).reshape(N, -1, 3)[:, self.N0:]
            w2_yaw_loss = torch.tensor([1., 1., 0.], device=rs2.device, dtype=rs2.dtype).reshape(1,1, 3)
            rs2_yaw_loss = rs2 * w2_yaw_loss
            loss2 = loss + self.f_huber(rs2_yaw_loss)/(2**(k - self.min_N + 1))
        return loss2'''
    ...
    def forward_with_rotation_matrices_mask(self, xs, hat_xs):
        ...
        masks[masks < 1] = 0
        Xs = SO3.exp(xs[:, ::self.min_train_freq, :3].reshape(-1, 3).double()) # xs : already decimated dtheta rotmat in gt -> 16마다 뽑기
        hat_xs = self.dt*hat_xs.reshape(-1, 3).double() # compensated gyro * dt
        Omegas = SO3.exp(hat_xs[:, :3]) # compensated dtheta -> rotmat
        # compute increment at min_train_freq by decimation
        for k in range(self.min_N):
            Omegas = Omegas[::2].bmm(Omegas[1::2]) # dR_0,16 dR_16,32 ...
        rs = SO3.log(bmmt(Xs, Omegas)).reshape(N, -1, 3)[:, self.N0:]
        loss = self.f_huber(rs)
        # compute increment from min_train_freq to max_train_freq
        for k in range(self.min_N, self.max_N):
            Omegas = Omegas[::2].bmm(Omegas[1::2])
