...
class SeqDataset(Data.Dataset):
    def __init__(self, root, dataname, devive = 'cpu', name='Nav', duration=200, step_size=200, mode='inference', 
        ...
        #print('seqdataset key ', self.data.keys())
        # dict_keys(['time', 'gyro', 'acc', 'gt_time', 'pos', 'quat', 'b_acc', 'b_gyro', 'velocity', 'rot_wxyz', 'gt_orientation', 'gt_translation', 'dt', 'mask', 'acc_cov', 'gyro_cov'])
        start_frame = 0
...
class SeqeuncesDataset(Data.Dataset):
    ...
    def construct_index_map(self, conf, data_root, data_name, seq_id):
        ...
        elif self.mode == 'train_1m': # 200Hz -> 1min 12000 frame
            end_frame = 12000
        ...
        if self.mode == "inference": # inference -> 총 길이만큼 window,step -> 단 한 번의 계산
            window_size = seq_len
            step_size = seq_len
