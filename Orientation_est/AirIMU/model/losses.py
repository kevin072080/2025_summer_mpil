...
def loss_(fc, pred, targ, sampling = None, dtype = 'trans'):
    ...
    if dtype == 'rot':
        dist = (pred * targ.Inv()).Log() # Denoising과는 순서가 반대지만, log 씌운 후  norm 구하면 각도 계산이니까 동일
    else:
        dist = pred - targ
    loss = fc(dist)
    return loss, dist
...
def get_loss(inte_state, data, confs):
    ...
    state_losses['rot'] = rot_dist[:,-1,:].norm(dim=-1).mean() # norm의 평균
    state_losses['rot_AOE'] = torch.sqrt((rot_dist[:, -1, :] ** 2).mean()) # norm RMS == AOE
    ...
    loss += (confs.pos_weight * pos_loss + confs.rot_weight * rot_loss)
    # report_hasNan(loss)
...
def get_RMSE(inte_state, data):
    ...
    rot_loss = _RMSE(dist_rot)[None,...] # AOE

    ## Relative pos error
    return {'pos': pos_loss, 'rot': rot_loss,
