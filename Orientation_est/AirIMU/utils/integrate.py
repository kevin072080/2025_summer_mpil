...
def integrate(integrator, loader, init, device="cpu", gtinit=False, save_full_traj=False, use_gt_rot=True):
    ...
    out_state['timestamp'] = torch.cat(timestamp, dim = -1)
    #  print('ts ', timestamp.shape)
    #out_state['vel'] = torch.cat(vel, dim=-2)

    #out_state['vel_gt'] = torch.cat(vel_gt, dim=-2)

    #print('before ori shape ', len(orientations))
    out_state['orientations'] = torch.cat(orientations, dim=-2) #ori shape  torch.Size([1, 28712, 4]) # xyzw
    #print('ori shape ',out_state['orientations'].shape)
    ...
    #print('ts shape ', len(timestamp), out_state['timestamp'].shape, (out_state['vel']).shape, out_state['poses'].shape, out_state['orientations'].shape, len(orientations))
    return out_state