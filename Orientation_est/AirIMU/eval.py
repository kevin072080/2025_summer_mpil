...
def get_metrics(eval_state):
    ...
    metrics['pos'], metrics['rot'] = eval_state['loss']['pos'].mean().item(), eval_state['loss']['rot'].mean().item()
...
def evaluate(network, loader, confs, silent_tqdm=False):
    ...
    with torch.no_grad():
        ...
        print("evaluating: position loss %f, rotation loss (=AOE) %f"\
                %(loss_states['pos'].mean(), loss_states['rot'].mean()))
