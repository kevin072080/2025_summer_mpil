...
def train(network, loader, confs, epoch, optimizer):
    ...
    for i, (data, init_state, label) in enumerate(t_range):
        ...
        # {**out_state, 'correction_acc': inference_state['correction_acc'], 'correction_gyro': inference_state['correction_gyro'],
        #                                 'corrected_acc': data['corrected_acc'], 'corrected_gyro': data['corrected_gyro']}
        loss_state = get_loss(inte_state, label, confs)
        AOE = loss_state['rot_AOE'].item() # 임의 추가, train에는 필요 없긴 할 듯
        ...
        if confs.propcov: # whether update cov or not
            ...
            t_range.set_description(f'training epoch: %03d, losses: %.06f, position, %.06f rotation %.06f'%(epoch, \
                                    loss_state['loss'], (pos_losses/(i+1)), (rot_losses/(i+1))))
...
def test(network, loader, confs):
    ...
    with torch.no_grad():
        ...
        t_range = tqdm.tqdm(loader)
        for i, (data, init_state, label) in enumerate(t_range): # 한 시퀀스에서 batch 옮기기

            data, init_state, label = move_to([data, init_state, label], confs.device)
            inte_state = network(data, init_state)

            loss_state = get_loss(inte_state, label, confs)
            AOE = loss_state['rot_AOE'].item()
            ...
            if confs.propcov: # optimizer.step() X -> param. update는 아님, loss 계산 위한 cov update
            ...
            t_range.set_description(f'testing losses: %.06f, position, %.06f rotation %.06f'%(losses/(i+1), \
                pos_losses/(i+1), rot_losses/(i+1)))
            t_range.refresh() # tqdm description update, 불필요
    ...
    return {"loss": (losses/(i+1)), "pos_loss":(pos_losses/(i+1)), "rot_loss":(rot_losses/(i+1)),
            "pred_cov_rot": (pred_cov_rot/(i+1)), "pred_cov_pos": (pred_cov_pos/(i+1)), "acc_covs": acc_covs, "gyro_covs": gyro_covs,
            }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='/home/mpil/miniconda3/envs/AirIMU/configs/exp/Astrobee/codenet_astrobee.conf', help='config file path')
