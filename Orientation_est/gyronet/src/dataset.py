...
def save_str_list(path: str | Path, items: list[str]):
    p = Path(path)
...
class BaseDataset(Dataset):
    def __init__(self, predata_dir, train_seqs, val_seqs, test_seqs, mode, N,
        ...
        self.N = N # power of 2, dynamic 필요
...
class EUROCDataset(BaseDataset):
    ...
    def read_data(self, data_dir):
        ...
        for sequence in sequences:
            ...
            # interpolate
            gt = self.interpolate(gt, gt[:, 0]/1e9, ts) # ts 기준으로 interp -> 뒤에서도 ts = ts
...
class ASTROBEEDataset(BaseDataset):
    ...
    def read_data(self, data_dir):
        ...
        f = os.path.join(self.predata_dir, 'a053.p')
...
class TUMVIDataset(BaseDataset):
    ...
    def read_data(self, data_dir):
        ...
        for sequence in sequences:
            ...
            mondict = {
                'xs': torch.cat((dxi_ij, masks.unsqueeze(1)), 1).float(), # xs : 16 ts 간격으로 각도 차이 구하기
                'us': imu.float(),
            }
