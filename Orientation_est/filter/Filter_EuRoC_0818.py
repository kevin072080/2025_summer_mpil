...
def Madgwick_EuRoc(loader, confs, resultdict_madg, orientations_madg):
    for i, sample in enumerate(loader):
        ...
        rot_gt = R.from_quat(quats_gt[:, [1, 2, 3, 0]]) # xyzw
...
def Mahony_EuRoc(loader, confs, resultdict_maho, orientations_maho):
    for i, sample in enumerate(loader):
        ...
        rot_gt = R.from_quat(quats_gt[:, [1, 2, 3, 0]])
        rot_gt_madglocal = R.from_matrix(R_gt_to_madg.cpu().numpy()) * R.from_quat(
            quats_gt[:, [1, 2, 3, 0]])  # Madg local -> Euroc GT
        ...
        # resultdict = metric(filename,  rot_est_aligned, rot_gt_madglocal, show=True)
        resultdict = metric(filename, final_rot_est, rot_gt, show=False)
        orientations = {
            'name': filename,
            'timestamp': ts.cpu().tolist(),
            'qx': R.as_quat(final_rot_est)[:, 0].tolist(),
...
save_path_maho = "/home/mpil/miniconda3/envs/filter/results/Mahony_EuRoC_0814"
