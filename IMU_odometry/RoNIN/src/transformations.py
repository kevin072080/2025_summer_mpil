...
def change_cf(ori, vectors):
    ...
    if len(ori.shape) == 1:
        ...
        #ori = np.repeat([ori], vectors.shape[0])
    q_s = ori[:, :1]
    q_r = ori[:, 1:]
...
class RandomHoriRotate:
    ...
    def __call__(self, feat, targ, **kwargs):
        ...
        rm = np.array([[math.cos(angle), -math.sin(angle)],[math.sin(angle), math.cos(angle)]])
        rm_3d = np.array([
        ...
        #print(feat_aug.shape, targ_aug.shape)
        #print(feat_aug[:, 3:6].shape, feat_aug[:, :3].shape)
        #feat_aug[:, :3] = np.matmul(rm_3d, feat[:, :3].T).T # 2-> 3, feat의 모든 행, 1~3 열
        #feat_aug[:, 3:6] = np.matmul(rm_3d, feat[:, 3:6].T).T # 3:5 -> 3:6
        feat_aug[:, :2] = np.matmul(rm, feat[:, :2].T).T
        feat_aug[:, 3:5] = np.matmul(rm, feat[:, 3:5].T).T
        targ_aug[:3] = np.matmul(rm_3d, targ[:3].T).T
