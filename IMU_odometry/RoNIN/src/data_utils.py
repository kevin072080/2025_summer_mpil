...
#from data_glob_speed import AstrobeeGlobSpeedSequence
...
def load_cached_sequences(seq_type, root_dir, ori_dir, data_list, cache_path, **kwargs):
    grv_only = kwargs.get('grv_only', True)
    grv_only = False # ?
    #seq_type = Astrobeedataset
    ...
    for i in range(len(data_list)):
        #print(osp.join(cache_path, data_list[i] + '.hdf5'))
        #print(i, osp.exists(osp.join(cache_path, data_list[i] + '.hdf5')))
        if True and cache_path is not None and osp.exists(osp.join(cache_path, data_list[i] + '.hdf5')):
        #if False:
            ...
            #print('else')
            if ori_dir != None:
                print('Estimated ori be used')
                seq = seq_type(osp.join(root_dir, data_list[i]), osp.join(ori_dir, data_list[i], data_list[i]+"_ori_q.txt"),  **kwargs)
                feat, targ, aux = seq.get_feature(), seq.get_target(), seq.get_aux()
            else:
                print('Estimated ori did not be used')
            ...
            if cache_path is not None and osp.isdir(cache_path):
                with h5py.File(osp.join(cache_path, data_list[i] + '.hdf5'), 'x') as f:
                    print('saved cache on ', osp.join(cache_path, data_list[i] + '.hdf5'))
                    f['feature'] = feat
                    f['target'] = targ
                    f['aux'] = aux
