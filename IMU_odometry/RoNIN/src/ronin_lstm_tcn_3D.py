...
_nano_to_sec = 1e09
_input_channel, _output_channel = 6, 3 # 2-> 3
device = 'cpu'
...
def train(args, **kwargs):
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            #ndim : batch, ts, channels / retain_axis : 각 채널(index 2의 축은 고정, retain)별 MSE, output_channel : 3D
            #utils.MSEAverageMeter 참고
            train_loss = 0
            start_t = time.time()

            #for bid, batch in enumerate(train_loader): # bid : index, batch : value
            for batch_id, (feat, targ, _, _) in enumerate(train_loader):

                feat, targ = feat.to(device), targ.to(device)
                optimizer.zero_grad()
                predicted = network(feat)
                #print(batch_id, targ.shape, feat.shape, predicted.shape)
                #1044 torch.Size([72, 400, 3]) torch.Size([72, 400, 6]) torch.Size([72, 400, 3]) -> dim 3
                #batch_size, timestamp, channels
                train_vel.add(predicted.cpu().detach().numpy(), targ.cpu().detach().numpy())
                # batch, time에 대해 MSE, vx, vy, vz는 그대로 (retain)
...
def recon_traj_with_preds_global(dataset, preds, ind=None, seq_id=0, type='preds', **kwargs):
    ...
    if type == 'gt':
        pos = dataset.gt_pos[seq_id][:, :3] # 2-> 3
    else:
        ts = dataset.ts[seq_id]
        # Compute the global velocity from local velocity.
        dts = np.mean(ts[ind[1:]] - ts[ind[:-1]])
        pos = preds * dts
        pos[0, :] = dataset.gt_pos[seq_id][0, :3] # 2-> 3
...
def test(args, **kwargs):
    ...
    losses_vel = MSEAverageMeter(2, [1], _output_channel) # 2, [1] -> 3, [2] 바꾸면 안 됨
    ate_all, rte_all = [], []
    pred_per_min = 200 * 60
    ...
    for idx, data in enumerate(test_data_list):
        ...
        # print(vel.shape, preds.shape)
        # (118606, 3)(118606, 3) -> (timestamp, channels) -> MSEavg -> ndim 2, retain_axis [1]이어야 함
        vel_losses = np.mean((vel - preds) ** 2, axis=0)
        losses_vel.add(vel, preds)
        # batch_size, timestamp, channels(x, y, z) -> 이 셋 중 vx,vy,vz의 시간 및 batch에 따른 MSE avg를 도출
...
if __name__ == '__main__':
    ...
    """
    default_config_file = osp.abspath(osp.join(osp.abspath(__file__), '/home/mpil/PycharmProjects/RoNIN/config/temporal_model_defaults.json'))
