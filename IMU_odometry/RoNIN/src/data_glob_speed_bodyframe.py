...
class GlobSpeedSequence(CompiledSequence):
    ...
    def load(self, data_path):
        ...
        #print(ori_q.shape[0], start_frame, ori_q.shape[0]-start_frame)
        #print('load에서의 ori, 10마다 : ', [ori_q[start_frame + 10*i] for i in range(2)])
        ...
        body_v = quaternion.as_float_array(ori_body_q[self.w:] * glob_v_q * ori_body_q[self.w:].conj())
        body_v = body_v[:, 1:]
        # 각 v마다 ori_body_q로 회전, 처음 항은 200~0 사이의 v이므로 그 가운데인 100번째 ori를 활용
...
class DenseSequenceDataset(Dataset):
    def __init__(self, seq_type, root_dir, data_list, cache_path=None, step_size=10, window_size=200,
        ...
        for i in range(len(data_list)):
            ...
            self.index_map += [[i, j] for j in range(window_size, self.targets[i].shape[0], step_size)] # 200부터 10 간격으로 ~ targets 시계열 수
