...
class TCNSeqNetwork(torch.nn.Module):
    def __init__(self, input_channel, output_channel, kernel_size, layer_channels, dropout=0.2):
        ...
        self.num_layers = len(layer_channels) # [32, 64, 128, 256, 72, 36] in config file
    ...
    def forward(self, x):
        ...
        # 1044 torch.Size([72, 400, 3]) torch.Size([72, 400, 6]) torch.Size([72, 400, 3]) -> x = (batch size, timestamp, channels)
        out = x.transpose(1, 2) # x = (batch size, channels, timestamp) -> (b, t, c)
        out = self.net(out)
