...
def collate_fn(batch):
    # 최대 길이 계산 (배치 내에서)
    max_len = max([len(x[0][0]) for x in batch])  # x[0]이 feature.T 텐서라고 가정
    #print('lll', max_len)
    # 배치 내 시퀀스의 길이를 맞추기 위해 패딩을 적용
    padded_batch = []
    target_batch = []
    seq_id_batch = []
    frame_id_batch = []
    ...
    for x in batch:
        # x[0]은 feature.T, x[1]은 target, x[2]는 seq_id, x[3]는 frame_id
        feature_tensor = torch.tensor(x[0])  # feature.T 텐서
        target_tensor = torch.tensor(x[1])  # target 텐서 (예시로 텐서로 변환)
        seq_id_tensor = torch.tensor(x[2])  # seq_id 텐서 (예시로 텐서로 변환)
        frame_id_tensor = torch.tensor(x[3])  # frame_id 텐서 (예시로 텐서로 변환)
        #print(feature_tensor.shape, target_tensor.shape)
        '''if max_len != feature_tensor.shape[1]:
            print('padding constructed')'''

        padded_feature = F.pad(feature_tensor, (0, max_len - feature_tensor.shape[1], 0, 0))
        #padded_target = F.pad(target_tensor, (0, 0, 0, max_len - target_tensor.shape[0]))

        # 패딩된 feature 텐서를 배치에 추가
        padded_batch.append(padded_feature)
        #target_batch.append(padded_target)
        target_batch.append((target_tensor))
        seq_id_batch.append(seq_id_tensor)
        frame_id_batch.append(frame_id_tensor)

    # 모든 텐서가 동일한 크기를 가지도록 스택
    padded_batch = torch.stack(padded_batch, 0)
    target_batch = torch.stack(target_batch, 0)
    #print(padded_batch.shape, target_batch.shape)
    # 결과를 리스트 형태로 반환
    return [padded_batch, target_batch, seq_id_batch, frame_id_batch]


_input_channel, _output_channel = 6, 3 #output channel 2 -> 3
_fc_config = {'fc_dim': 512, 'in_dim': 7, 'dropout': 0.5, 'trans_planes': 128}
# fc_dim은 hyperparam. -> 왜 resnet18은 1024가 아닌지는 모름
# in_dim = window_size // 32 + 1, window_size default == 100 -> in_dim default 4
...
def get_dataset(root_dir, data_list, args, **kwargs):
    ...
    elif args.dataset == 'astrobee':
        seq_type = AstrobeeGlobSpeedSequence
...
def get_dataset_astrobee(root_dir, ori_dir, data_list, args, **kwargs):
    mode = kwargs.get('mode', 'train')
    ...
    if mode == 'train': # random shuffle 및 shift, transform 통해 섞어서 data num 증가
        random_shift = args.step_size // 2
        shuffle = True
        # transforms = RandomHoriRotate(math.pi * 2) #Random HACF -> 2D일 때와 3D일 때 다름  # g align이 안 되어 있어서 위험해보
    elif mode == 'val': # shuffle만 적용하여 training이 그 세트에 대해 valid한지 여부 판단
        shuffle = True
    elif mode == 'test': #game rotation vector만 사용하여 시험
        shuffle = False
...
def get_dataset_from_list_astrobee(root_dir, ori_dir, list_path, args, **kwargs):
    with open(list_path) as f:
        data_list = [s.strip().split(',' or ' ')[0] for s in f.readlines() if len(s) > 0 and s[0] != '#']
    return get_dataset_astrobee(root_dir, ori_dir, data_list, args, **kwargs)
...
def train(args, **kwargs):
    ...
    start_t = time.time()
    if args.dataset == 'astrobee':
        train_dataset = get_dataset_from_list_astrobee(args.root_dir,  args.ori_dir, args.train_list, args, mode='train')
        # feat.astype(np.float32).T, targ.astype(np.float32), seq_id, frame_id
    ...
    if args.val_list is not None:
        val_dataset = get_dataset_from_list_astrobee(args.root_dir, args.ori_dir, args.val_list, args, mode='val')
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.1, patience=10, eps=args.lr/1e-6)
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            for batch_id, (feat, targ, _, _) in enumerate(train_loader):
                ...
                # print('train 과정 ', batch_id, feat.shape, targ.shape, pred.shape)
                # print(batch_id, feat[0], targ[0], pred[0])
                train_outs.append(pred.cpu().detach())
                train_targets.append(targ.cpu().detach())
                # print(batch_id, step, pred.shape, targ.shape)
                step += 1
            train_outs = torch.cat(train_outs,dim=0) # 배치별 계산 결과 이어붙이기
            train_targets = torch.cat(train_targets, dim=0)
            train_losses = torch.mean((train_outs - train_targets) ** 2, dim=0)

            end_t = time.time()
            print('-------------------------')
            print('Epoch {}, time usage: {:.3f}s, average loss: {}/{:.6f}'.format(
                epoch, end_t - start_t, train_losses.cpu().numpy().tolist(), torch.mean(train_losses).item()))
            train_losses_all.append(torch.mean(train_losses).item())

            if summary_writer is not None:
                add_summary(summary_writer, train_losses.cpu().numpy(), epoch + 1, 'train')
...
def recon_traj_with_preds(dataset, preds, seq_id=0, **kwargs):
    ...
    # print(dataset.index_map) #[[0, 0], [0, 10], [0, 20], [0, 30], [0, 40], [0, 50], [0, 60], [0, 70]
    dts = np.mean(ts[ind[1:]] - ts[ind[:-1]]) # 0.11822206538756089, 기존 dataset dt = 0.01
    # => 10 단위로 traj
    pos = np.zeros([preds.shape[0] + 2, 3]) # 2-> 3, 첫 2는 시작 및 끝 보간용
    pos[0] = dataset.gt_pos[seq_id][0, :3]  # 2-> 3, 초기 위치를 gt 값에서 얻어 옴, 0,0
    ...
    print(len(ts), len(ts_ext), len(pos))
    print(ts[0], ts[-1])
    print(ts_ext[0], ts_ext[-1])
    print(pos[0], pos[-1])
...
def recon_traj_with_acc(dataset, seq_id=0, **kwargs):
    ts = dataset.ts[seq_id]  # timestamp
    ind = np.array([i[1] for i in dataset.index_map if i[0] == seq_id], dtype=int)
    acc = dataset.features[seq_id][ind,3:6]
    dts = np.mean(ts[ind[1:]] - ts[ind[:-1]])
    v = np.zeros([acc.shape[0] + 2, 3])
    v[0] = dataset.targets[seq_id][0, :3]
    v[1:-1] = np.cumsum(acc[:, :3] * dts, axis=0) + v[0]
    v[-1] = v[-2]
    ts_ext = np.concatenate([[ts[0] - 1e-06], ts[ind], [ts[-1] + 1e-06]], axis=0)  # 0, n 변위 임의 추가


    pos = np.zeros([v.shape[0] + 2, 3])
    pos[0] = dataset.gt_pos[seq_id][0, :3]  # 2-> 3, 초기 위치를 gt 값에서 얻어 옴, 0,0
    pos[1:-1] = np.cumsum(v[:, :3] * dts, axis=0) + pos[0]  # 2-> 3, 변위 누적하는 부분임, 1에서 n-1까지
    pos[-1] = pos[-2]
    ts_ext = np.concatenate([[ts[0] - 2e-06], [ts[0] - 1e-06], ts[ind], [ts[-1] + 1e-06],[ts[-1] - 2e-06]], axis=0)  # 0, n 변위 임의 추가
...
def test_sequence(args):
    ...
    #ori_dir0 = osp.join(args.ori_dir, test_data_list[0] +"_ori_q.txt")
    _ = get_dataset_astrobee(root_dir, args.ori_dir, [test_data_list[0]], args)
    ...
    for data in test_data_list:
        #ori_dir = osp.join(args.ori_dir, data + "_ori_q.txt")
        seq_dataset = get_dataset_astrobee(root_dir, args.ori_dir, [data], args, mode='test')
        seq_loader = DataLoader(seq_dataset, batch_size=1024, shuffle=False, collate_fn=collate_fn,drop_last=False) # batch size는 hyperparam. 너무 느린데 수정 필요?
        ...
        pos_naive = recon_traj_with_acc(seq_dataset)
        ...
        plt.plot(pos_pred[:, 0], pos_pred[:, 1])
        plt.plot(pos_gt[:, 0], pos_gt[:, 1])
        plt.plot(pos_naive[:, 0], pos_naive[:, 1])
        plt.title('X-Y')
        plt.axis('equal')
        plt.legend(['Predicted', 'Ground truth', 'NDI'])'''
        ...
        plt.plot(pos_pred[:, 0], pos_pred[:, 2])
        plt.plot(pos_gt[:, 0], pos_gt[:, 2])
        plt.plot(pos_naive[:, 0], pos_naive[:, 2])
        plt.title('X-Z')
        plt.legend(['Predicted', 'Ground truth', 'NDI'])'''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='astrobee', choices=['ronin', 'ridi', 'astrobee'])
    parser.add_argument('--max_ori_error', type=float, default=20.0)
    parser.add_argument('--step_size', type=int, default=10)
    parser.add_argument('--window_size', type=int, default=100)
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'test'])
    parser.add_argument('--lr', type=float, default=1e-6) ####################3
    parser.add_argument('--batch_size', type=int, default=128)
    parser.add_argument('--epochs', type=int, default=10000)
    parser.add_argument('--arch', type=str, default='resnet18') # resnet18 -> resnet101
