...
class BasicBlock1D(nn.Module):
    ...
    def forward(self, x):
        ...
        out += residual # skip connection -> back prop 때 grad vanishing 문제 해결
        out = self.relu(out)
...
class Bottleneck1D(nn.Module):
    expansion = 4 #bottleneck은 압축하여 계산하여 계산량 줄인 뒤 다시 확장, expansion : 확장 비
...
class FCOutputModule(nn.Module):
    ...
    def __init__(self, in_planes, num_outputs, **kwargs):
        ...
        fc_dim = kwargs.get('fc_dim', 1536) #512 units * 2D -> 512 * 3D
        dropout = kwargs.get('dropout', 0.5)
        in_dim = kwargs.get('in_dim', 7)
        trans_planes = kwargs.get('trans_planes', None)
        if trans_planes is not None: #trans_planes == 128, in_planes == 512
    ...
    def forward(self, x):
        if self.transition is not None:
            x = self.transition(x)
        x = x.view(x.size(0), -1)
        # print("x before fc:", x.shape)  # 기대: [B, C, T] 또는 [B, C*T]
...
class ResNet1D(nn.Module):
    def __init__(self, num_inputs, num_outputs, block_type, group_sizes, base_plane=64, output_block=None,
        ...
        # Residual groups, for resnet101 group_sizes == [3, 4, 23, 3]
        self.planes = [self.base_plane * (2 ** i) for i in range(len(group_sizes))] # [64, 128, 256, 512]
        kernel_size = kwargs.get('kernel_size', 3)
        strides = [1] + [2] * (len(group_sizes) - 1) # [1, 2, 2, 2]
        dilations = [1] * len(group_sizes) #[1, 1, 1, 1]
